<?php
    ...
    class DoradoWebDeactivate{
		...
		public function submit_and_deactivate(){
			$wd_options =  $this->config;
			$wd_opt_sub_deact = WDWTILibrary::get($wd_options->prefix . '_submit_and_deactivate','');
			if( !empty( $wd_opt_sub_deact ) ){
				if( WDWTILibrary::get($wd_options->prefix . '_submit_and_deactivate','') == 2 || WDWTILibrary::get($wd_options->prefix . "_submit_and_deactivate",'') == 3 ){
					$api = new DoradoWebApi( $wd_options );	
					$hash = $api->get_hash();
					if($hash != null){
						$data = array();
						
						$data["reason"] = WDWTILibrary::get( $wd_options->prefix . "_reasons",'');
						$data["site_url"] = site_url();
						$data["plugin_id"] = $wd_options->wd_plugin_id;
						
						$data["additional_details"] = WDWTILibrary::get( $wd_options->prefix . '_additional_details', '');
						$admin_data = wp_get_current_user();
						$data["email"] = WDWTILibrary::get( $wd_options->prefix . "_email", $admin_data->data->user_email);
				...
				if( WDWTILibrary::get( $wd_options->prefix . "_submit_and_deactivate", '') == 2 || WDWTILibrary::get( $wd_options->prefix . "_submit_and_deactivate",'') == 1 ){
					$deactivate_url = 
						add_query_arg(
							array(
								'action' => 'deactivate',
