...
class DoradoWebNotices {
    ...
    public function admin_notice_ignore() {
        $wd_options =  $this->config;
        $wd_opt_admin_notice = WDWTILibrary::get( $wd_options->prefix . '_admin_notice_ignore','' );
        // If user clicks to ignore the notice, update the option to not show it again
        if ( !empty( $wd_opt_admin_notice ) ) {
            $admin_notices_option = get_option( $wd_options->prefix . '_admin_notice', array() );
            $admin_notices_option[ WDWTILibrary::get( $wd_options->prefix . '_admin_notice_ignore','' ) ]['dismissed'] = 1;
            update_option( $wd_options->prefix . '_admin_notice', $admin_notices_option );
    ...
    public function admin_notice_temp_ignore() {
        $wd_options =  $this->config;
        $wd_opt_admin_notice_temp = WDWTILibrary::get( $wd_options->prefix . '_admin_notice_temp_ignore','');
        // If user clicks to temp ignore the notice, update the option to change the start date - default interval of 14 days
        if ( !empty( $wd_opt_admin_notice_temp ) ) {
            $admin_notices_option = get_option( $wd_options->prefix . '_admin_notice', array() );
            $current_date = current_time( "n/j/Y" );
            $date_array = explode( '/', $current_date );
            $interval = (int)WDWTILibrary::get('wd_int', 14);
            $date_array[1] += $interval;
            $new_start = date( "n/j/Y", mktime( 0, 0, 0, $date_array[0], $date_array[1], $date_array[2] ) );

            $admin_notices_option[WDWTILibrary::get( $wd_options->prefix . '_admin_notice_temp_ignore','')]['start'] = $new_start;
            $admin_notices_option[ WDWTILibrary::get( $wd_options->prefix . '_admin_notice_temp_ignore') ]['dismissed'] = 0;
            update_option( $wd_options->prefix . '_admin_notice', $admin_notices_option );
    ...
    public function admin_notice_pages($pages) {
        ...
        foreach ( $pages as $key => $page ) {
            if (is_array($page)) {
                if ( WDWTILibrary::get('page', '' ) == $page[0] && WDWTILibrary::get('tab','') == $page[1] ) {
                    return true;
                }
            }
            else {
