...
class WDTIModelTwitter_integration {
  ...
  public function get_rows_data() {
    ...
    $where = ((!empty( $search_value )) ? 'WHERE title LIKE "%' . $search_value . '%"' : '');
    $asc_or_desc = WDWTILibrary::get('asc_or_desc', 'asc');
    $order_by_value = WDWTILibrary::get('order_by', '');
    $order_by = ' ORDER BY ' . (!empty( $order_by_value ) ? '`'.( $order_by_value ) .'`' : 'id') . ' ' . $asc_or_desc;
    $page_number = (int) WDWTILibrary::get('page_number', '');
